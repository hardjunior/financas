...
$this->SetFont('Arial','I',7);
$this->Cell(0,0.2,utf8_decode("Origem de dados: Movimento Financeiro ".empresa.".   |     Usuário: $nome.    "),0,0,'L');
...
$qrv=mysqli_query($_SG['link'],"SELECT * FROM exclusoes WHERE conta_mov='$conta' && usuario_mov='$usuario' ORDER By id DESC");
if (mysqli_num_rows($qrv)!==0){

$qrg=mysqli_query($_SG['link'],"SELECT SUM(valor_mov) as total FROM exclusoes WHERE tipo_mov=1 && conta_mov='$conta' && usuario_mov='$usuario'");
$rowg=mysqli_fetch_array($qrg);
$entradas=$rowg['total'];

$qrg=mysqli_query($_SG['link'],"SELECT SUM(valor_mov) as total FROM exclusoes WHERE tipo_mov=0 && conta_mov='$conta' && usuario_mov='$usuario'");
$rowg=mysqli_fetch_array($qrg);
$saidas=$rowg['total'];
...
$pdf->Cell(11,0.6,utf8_decode('Descrição do movimento'),0,0,'C',1);
...
$pdf->Cell(3.7,0.5,utf8_decode($row2['nome']),0,0,'C',1);
if ($tipo==0){
$pdf->SetFont('Times','',9);
$pdf->SetTextColor(255,0,0);
$pdf->Cell(2.4,0.5,r_eur($valor),0,0,'L',1);}
if ($tipo==1){
$pdf->SetFont('Times','',9);
$pdf->SetTextColor(0,0,255);
$pdf->Cell(2.4,0.5,r_eur($valor),0,0,'L',1);}
...
$pdf->SetFont('Arial','',6);
$pdf->SetTextColor(255,0,0);
$pdf->Cell(3,0.5,r_eur($saidasexc),0,1,'R',0);}
if ($saidas=0){
$pdf->Cell(16,0.5,'',0,1,'R',0);}
