...
$qrs=mysqli_query($_SG['link'],"SELECT SUM(valor) as total FROM movimentos WHERE tipo=0 && conta=1 && usuario='$usuario' && mes='$mes' && ano='$ano'");
...
$qre=mysqli_query($_SG['link'],"SELECT dia, cat, SUM(valor) as total FROM movimentos WHERE tipo=1 && conta=1 && usuario='$usuario' && mes='$mes' && ano='$ano' GROUP BY cat ORDER BY dia");
...
$pdf->SetFont('Times','',10);
$pdf->SetTextColor(0,0,255);
$pdf->Cell(4,0.5,r_eur($valor),0,0,'L',1);
$pdf->SetTextColor(0,0,0);
...
//Estáticas de saídas
$qrs=mysqli_query($_SG['link'],"SELECT dia, cat, SUM(valor) as total FROM movimentos WHERE tipo=0 && conta=1 && usuario='$usuario' && mes='$mes' && ano='$ano' GROUP BY cat ORDER BY dia");
...
$pdf->SetFont('Arial','',12);
$pdf->SetTextColor(255,0,0);
$pdf->Cell(4,0.7,r_eur($valortotals),0,1,'R',1);
$pdf->Ln(1);
...
if  ($resultado_mes>=0){
	$pdf->SetFont('Arial','',12);
	$pdf->SetTextColor(0,0,215);
	$pdf->Cell(4,0.6,r_eur($balanco),0,1,'R',1);}
if  ($resultado_mes<0){
	$pdf->SetFont('Arial','',12);
	$pdf->SetTextColor(255,0,0);
	$pdf->Cell(4,0.6,r_eur($balanco),0,1,'R',1);}
...
if  ($resultado_geral>=0){
	$pdf->SetFont('Arial','',8);
	$pdf->SetTextColor(0,0,215);
	$pdf->Cell(4,0.6,r_eur($balancogeral),0,1,'R',0);}
if  ($resultado_geral<0){
	$pdf->SetFont('Arial','',8);
	$pdf->SetTextColor(255,0,0);
	$pdf->Cell(4,0.6,r_eur($balancogeral),0,1,'R',0);}
...
$qrv=mysqli_query($_SG['link'],"SELECT * FROM movimentos WHERE conta=1 && usuario='$usuario' && ano='$ano' ORDER By dia");
if (mysqli_num_rows($qrv)!==0){

$qr=mysqli_query($_SG['link'],"SELECT SUM(valor) as total FROM movimentos WHERE tipo=1 && conta=1 && usuario='$usuario' && ano='$ano'");
$row=mysqli_fetch_array($qr);
$entradas=$row['total'];
$entradastotal=formata_dinheiro($entradas);

$qrs=mysqli_query($_SG['link'],"SELECT SUM(valor) as total FROM movimentos WHERE tipo=0 && conta=1 && usuario='$usuario' && ano='$ano'");
$rows=mysqli_fetch_array($qrs);
$saidas=$rows['total'];

$qror=mysqli_query($_SG['link'],"SELECT SUM(valor) as total FROM orcamento WHERE conta=1 && usuario='$usuario' && ano='$ano'");
...
//Estáticas de entradas
$qre=mysqli_query($_SG['link'],"SELECT dia, cat, SUM(valor) as total FROM movimentos WHERE tipo=1 && conta=1 && usuario='$usuario' && ano='$ano' GROUP BY cat ORDER BY dia");
...
//Estáticas de saídas
$qrs=mysqli_query($_SG['link'],"SELECT dia, mes, cat, SUM(valor) as total FROM movimentos WHERE tipo=0 && conta=1 && usuario='$usuario' && ano='$ano' GROUP BY cat ORDER BY dia");
if(mysqli_num_rows($qrs)!==0){
...
$pdf->SetFont('Arial','',12);
$pdf->SetTextColor(255,0,0);
$pdf->Cell(7,0.7,r_eur($valortotals),'B',0,'L',1);
$pdf->SetTextColor(0,0,0);
$pdf->SetFont('Times','',10);
...
if  ($restante>=0){
	$pdf->SetFont('Arial','',12);
	$pdf->SetTextColor(0,0,215);
	$pdf->Cell(3,0.7,r_eur($restanteformatado),0,1,'R',1);}
if  ($restante<0){
	$pdf->SetFont('Arial','',12);
	$pdf->SetTextColor(255,0,0);
	$pdf->Cell(3,0.7,r_eur($restanteformatado),0,1,'R',1);}
...
$qrv=mysqli_query($_SG['link'],"SELECT * FROM movimentos WHERE conta='$conta' && usuario='$usuario' && mes='$mes' && ano='$ano' ORDER By dia");
if (mysqli_num_rows($qrv)!==0){

$qrs=mysqli_query($_SG['link'],"SELECT SUM(valor) as total FROM movimentos WHERE tipo=0 && conta='$conta' && usuario='$usuario' && mes='$mes' && ano='$ano'");
$rows=mysqli_fetch_array($qrs);
$saidas=$rows['total'];

//Estáticas de saídas
$qrs=mysqli_query($_SG['link'],"SELECT dia, cat, SUM(valor) as total FROM movimentos WHERE tipo=0 && conta='$conta' && usuario='$usuario' && mes='$mes' && ano='$ano' GROUP BY cat ORDER BY dia");
if(mysqli_num_rows($qrs)!==0){
...
$this->AliasNbPages();
$this->SetTextColor(0,0,0);
$this->Cell(0,10,utf8_decode(" Origem de dados: Movimento Financeiro Loja ".empresa.".   |   Usuário: $nome.    "),0,0,'L');
$this->Cell(0,10,utf8_decode("Página ").$this->PageNo().'/{nb}',0,0,'R');
}
}
...
$qrv=mysqli_query($_SG['link'],"SELECT * FROM movimentos WHERE tipo=0 && conta='$conta' && usuario='$usuario' && ano='$ano' ORDER By dia");
if (mysqli_num_rows($qrv)!==0){

$qrs=mysqli_query($_SG['link'],"SELECT SUM(valor) as total FROM movimentos WHERE tipo=0 && conta='$conta' && usuario='$usuario' && ano='$ano'");
$rows=mysqli_fetch_array($qrs);
$saidas=$rows['total'];

//Estáticas de saídas
$qrs=mysqli_query($_SG['link'],"SELECT dia, cat, SUM(valor) as total FROM movimentos WHERE tipo=0 && conta='$conta' && usuario='$usuario' && ano='$ano' GROUP BY cat ORDER BY dia");
if(mysqli_num_rows($qrs)!==0){
...
$cat=$row['cat'];
$qr2=mysqli_query($_SG['link'],"SELECT nome FROM categorias WHERE id='$cat' ORDER BY nome");
$row2=mysqli_fetch_array($qr2);
$categoria=$row2['nome'];
$valorcat=$row['total'];
$percento = @round (($valorcat/$saidas) * 100,1);
